    ...
    <div className="container">
      ...
      <div className="filters-wrap">
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="reset-db-btn" disabled={isSeeding} onClick={() => handleReseed(2000)}>
            {isSeeding ? 'Seeding...' : 'Reseed 2K'}
          </button>
          <button className="reset-db-btn" disabled={isSeeding} onClick={() => handleReseed(5000)}>
      ...
      <div className="grid3">
        <div className="panel">
          <h3>Vehicle Type share {filters.vehicleType && <span style={{ color: 'var(--accent)' }}>(Filter: {filters.vehicleType})</span>}</h3>
          <BarList
      ...
      <div className="grid">
        <div className="panel">
          <h3>Payment Method — completed rides {filters.paymentMethod && <span style={{ color: 'var(--accent)' }}>(Filter: {filters.paymentMethod})</span>}</h3>
          <BarList
      ...
      <div className="grid">
        <BookingSimulator onSimulateComplete={() => { fetchStats(); fetchBookingsList(); }} />
            ...
            <form onSubmit={handleUpdateBooking} className="modal-form">
              ...
              {editingBooking.status === 'Completed' && (
                <>
                  <div className="form-group">
                    ...
                    <input type="number" min="1" max="5" step="0.1" className="form-input" 
                      value={editingBooking.driverRating === null || editingBooking.driverRating === undefined ? '' : editingBooking.driverRating} 
                      onChange={(e) => setEditingBooking(prev => ({ ...prev, driverRating: e.target.value }))} required />
                  </div>
                  <div className="form-group">
                    ...
                    <input type="number" min="1" max="5" step="0.1" className="form-input" 
                      ...
                      onChange={(e) => setEditingBooking(prev => ({ ...prev, customerRating: e.target.value }))} required />
                  </div>
                </>
              )}
              ...
              {editingBooking.status === 'Cancelled by Driver' && (
                <div className="form-group">
                  ...
                  <select className="form-select" value={editingBooking.driverCancellationReason || ''} 
                    onChange={(e) => setEditingBooking(prev => ({ ...prev, driverCancellationReason: e.target.value }))} required>
              ...
              {editingBooking.status === 'Cancelled by Customer' && (
                <div className="form-group">
                  ...
                  <select className="form-select" value={editingBooking.custCancellationReason || ''} 
                    onChange={(e) => setEditingBooking(prev => ({ ...prev, custCancellationReason: e.target.value }))} required>
                    <option value="">-- Select Reason --</option>
      ...
      <footer>
        ...
        &nbsp;|&nbsp; <a href="/" style={{ color: 'var(--accent)' }}>← Passenger App</a>
      </footer>
    </div>
  );
}
